...
class World: #are the walls built in??
    ...
    def setRoom(self, data):
        """ Sets self.room to a deepcopy of data and updates position of bot. If bot generates in wall, sets bot to random position.
        """
...
def expandTopPopulation(slist):
    """ Takes a sorted list of programs, takes the top FRACTOKEEP of them, and generates the rest by crossing over the selected population.
        Iterates through all children and mutates once with MUTATEPROB. 
    """
    toptoKeep = []
...
def saveToFile(filename, p, comment = ""):
   """Saves the data from Program p
           to a file named filename. Comment can give fitness"""
   f = open(filename, "w")
   if comment != "":
